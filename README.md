...
GitHub Actions 的日常 PR 只运行 `:app:testDebugUnitTest` 和 `:app:lintDebug`，不自动构建 APK。需要 release APK 时，先推送 `v*` 版本 tag；workflow 会要求配置 Android release signing secrets，并上传 `lifelab-release-apk` artifact，保留 7 天。
